  ...
  auto set_corner = [&]( int row_init,
    ...
    for ( int row = row_init; ( increment_row ? row < row_end : row > row_end ); increment_row ? row++ : row-- ) {
      for ( int col = col_init; ( increment_col ? col < col_end : col > col_end ); increment_col ? col++ : col-- ) {
        if ( maze_[row][col] & Thread_maze::path_bit_ ) {
...
void Thread_solvers::solve_with_dfs_threads()
{
  ...
  if ( game_ == Maze_game::hunt ) {
    ...
    for ( int i = 0; i < num_threads_; i++ ) {
      const Thread_paint& thread_mask = thread_masks_[i];
      const Thread_maze::Point& corner = corner_starts_[random_starting_corners[i]];
...
void Thread_solvers::dfs_thread_hunt( Thread_maze::Point start, int thread_index, Thread_paint paint )
{
  ...
  std::vector<Thread_maze::Point>& dfs = thread_paths_[thread_index];
  dfs.push_back( start );
  ...
  while ( !dfs.empty() ) {
    ...
    do {
      ...
      solver_mutex_.lock();
      bool push_next
        = !( maze_[next.row][next.col] & seen ) && ( maze_[next.row][next.col] & Thread_maze::path_bit_ );
      solver_mutex_.unlock();
      if ( push_next ) {
...
void Thread_solvers::dfs_thread_hunt_animated( Thread_maze::Point start, int thread_index, Thread_paint paint )
{
  Thread_cache seen = paint << thread_tag_offset_;
  std::vector<Thread_maze::Point>& dfs = thread_paths_[thread_index];
  dfs.push_back( start );
...
void Thread_solvers::dfs_thread_gather( Thread_maze::Point start, int thread_index, Thread_paint paint )
{
  Thread_cache seen = paint << thread_tag_offset_;
  std::vector<Thread_maze::Point>& dfs = thread_paths_[thread_index];
  dfs.push_back( start );
...
void Thread_solvers::dfs_thread_gather_animated( Thread_maze::Point start, int thread_index, Thread_paint paint )
{
  Thread_cache seen = paint << thread_tag_offset_;
  std::vector<Thread_maze::Point>& dfs = thread_paths_[thread_index];
  dfs.push_back( start );
...
void Thread_solvers::randomized_dfs_thread_hunt( Thread_maze::Point start, int thread_index, Thread_paint paint )
{
  Thread_cache seen = paint << thread_tag_offset_;
  std::vector<Thread_maze::Point>& dfs = thread_paths_[thread_index];
  dfs.push_back( start );
...
void Thread_solvers::randomized_dfs_thread_hunt_animated( Thread_maze::Point start,
                                                          int thread_index,
                                                          Thread_paint paint )
{
  Thread_cache seen = paint << thread_tag_offset_;
...
void Thread_solvers::randomized_dfs_thread_gather( Thread_maze::Point start, int thread_index, Thread_paint paint )
{
  Thread_cache seen = paint << thread_tag_offset_;
  std::vector<Thread_maze::Point>& dfs = thread_paths_[thread_index];
  dfs.push_back( start );
...
void Thread_solvers::randomized_dfs_thread_gather_animated( Thread_maze::Point start,
                                                            int thread_index,
                                                            Thread_paint paint )
{
  Thread_cache seen = paint << thread_tag_offset_;
...
void Thread_solvers::solve_with_bfs_threads()
{
  ...
  if ( game_ == Maze_game::gather ) {
    ...
    int thread = 0;
    for ( const std::vector<Thread_maze::Point>& path : thread_paths_ ) {
      Thread_paint single_color = thread_masks_[thread++];
      const Thread_maze::Point& p = path.front();
    ...
    for ( const Thread_maze::Point& p : thread_paths_[escape_path_index_] ) {
      maze_[p.row][p.col] &= ~thread_mask_;
...
void Thread_solvers::bfs_thread_hunt( Thread_maze::Point start, int thread_index, Thread_paint paint )
{
  ...
  seen[start] = { -1, -1 };
  My_queue<Thread_maze::Point>& bfs = thread_queues_[thread_index];
  bfs.push( start );
  Thread_maze::Point cur = start;
  while ( !bfs.empty() ) {
    // Lock? Garbage read stolen mid write by winning thread is still ok for program logic.
...
void Thread_solvers::bfs_thread_hunt_animated( Thread_maze::Point start, int thread_index, Thread_paint paint )
{
  // This will be how we rebuild the path because queue does not represent the current path.
  std::unordered_map<Thread_maze::Point, Thread_maze::Point>& seen = thread_bfs_maps[thread_index];
  seen[start] = { -1, -1 };
...
void Thread_solvers::bfs_thread_gather( Thread_maze::Point start, int thread_index, Thread_paint paint )
{
  std::unordered_map<Thread_maze::Point, Thread_maze::Point>& seen = thread_bfs_maps[thread_index];
...
void Thread_solvers::bfs_thread_gather_animated( Thread_maze::Point start, int thread_index, Thread_paint paint )
{
  std::unordered_map<Thread_maze::Point, Thread_maze::Point>& seen = thread_bfs_maps[thread_index];
  Thread_cache seen_bit = paint << 4;
  seen[start] = { -1, -1 };
...
Thread_maze::Point Thread_solvers::pick_random_point()
{
  Thread_maze::Point choice = { row_random_( generator_ ), col_random_( generator_ ) };
  if ( !( maze_[choice.row][choice.col] & Thread_maze::path_bit_ ) || maze_[choice.row][choice.col] & finish_bit_
...
Thread_maze::Point Thread_solvers::find_nearest_square( Thread_maze::Point choice )
{
  // Fanning out from a starting point should work on any medium to large maze.
  for ( const Thread_maze::Point& p : all_directions_ ) {
    Thread_maze::Point next = { choice.row + p.row, choice.col + p.col };
    if ( next.row > 0 && next.row < maze_.row_size() - 1 && next.col > 0 && next.col < maze_.col_size() - 1
         && ( maze_[next.row][next.col] & Thread_maze::path_bit_ ) && !( maze_[next.row][next.col] & start_bit_ )
  ...
  for ( int row = 1; row < maze_.row_size() - 1; row++ ) {
    for ( int col = 1; col < maze_.col_size() - 1; col++ ) {
      if ( ( maze_[row][col] & Thread_maze::path_bit_ ) && !( maze_[row][col] & start_bit_ )
...
{
  escape_path_index_ = -1;
  for ( std::vector<Thread_maze::Point>& vec : thread_paths_ ) {
    vec.clear();
