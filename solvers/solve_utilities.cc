...
constexpr std::array<uint16_t, 4> thread_bits
...
constexpr std::string_view ansi_no_solution
    = "\033[38;5;15m\033[48;255;0;0m╳ no thread won..\033[0m";
...
struct Bfs_monitor
{
    ...
    std::vector<std::vector<Maze::Point>> thread_paths;
    Bfs_monitor()
        : thread_maps{num_threads}, thread_queues{num_threads},
...
is_valid_start_or_finish(const Maze::Maze &maze, const Maze::Point &choice)
{
    return choice.row > 0 && choice.row < maze.row_size() - 1 && choice.col > 0
           ...
           && !(maze[choice.row][choice.col] & finish_bit)
...
void
print_hunt_solution_message(uint16_t winning_index)
{
    ...
    std::cout << (thread_colors.at(thread_bits.at(winning_index)))
              << " thread won!\n";
}
...
void
deluminate_maze(Maze::Maze &maze)
{
    for (int row = 0; row < maze.row_size(); ++row)
    {
        for (int col = 0; col < maze.col_size(); ++col)
...
print_overlap_key()
{
    std::cout << "┌────────────────────────────────────────────────────────────"
              ...
              << "├────────────┬────────────┬────────────┬────────────┬────────"
                 "────┤\n"
              << "│ " << thread_colors.at(1) << " = 0      │ "
              << thread_colors.at(2) << " = 1      │ " << thread_colors.at(3)
              << " = 1|0    │ " << thread_colors.at(4) << " = 2      │ "
              << thread_colors.at(5) << " = 2|0    │\n"
              << "├────────────┼────────────┼────────────┼────────────┼────────"
                 "────┤\n"
              << "│ " << thread_colors.at(6) << " = 2|1    │ "
              << thread_colors.at(7) << " = 2|1|0  │ " << thread_colors.at(8)
              << " = 3      │ " << thread_colors.at(9) << " = 3|0    │ "
              << thread_colors.at(10) << " = 3|1    │\n"
              << "├────────────┼────────────┼────────────┼────────────┼────────"
                 "────┤\n"
              << "│ " << thread_colors.at(11) << " = 3|1|0  │ "
              << thread_colors.at(12) << " = 3|2    │ " << thread_colors.at(13)
              << " = 3|2|0  │ " << thread_colors.at(14) << " = 3|2|1  │ "
