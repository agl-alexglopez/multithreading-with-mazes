...
constexpr std::array<Speed::Speed_unit, 8> builder_speeds = { 0, 5000, 2500, 1000, 500, 250, 100, 1 };

/* * * * * * * * * * * * * * *      Cout Printing Functions      * * * * * * * * * * * * * * * * */

void print_square( const Maze::Maze& maze, const Maze::Point& p )
{
  const Maze::Square& square = maze[p.row][p.col];
  if ( square & Maze::markers_mask ) {
    const Maze::Backtrack_marker mark
      = static_cast<Maze::Backtrack_marker>( square & Maze::markers_mask ) >> Maze::marker_shift;
...
void clear_and_flush_grid( const Maze::Maze& maze )
{
...
void flush_cursor_maze_coordinate( const Maze::Maze& maze, const Maze::Point& p )
{
...
void print_maze_square( const Maze::Maze& maze, const Maze::Point& p )
{
  const Maze::Square& square = maze[p.row][p.col];
...
bool can_build_new_square( const Maze::Maze& maze, const Maze::Point& next )
{
  return next.row > 0 && next.row < maze.row_size() - 1 && next.col > 0 && next.col < maze.col_size() - 1
         && !( maze[next.row][next.col] & Maze::builder_bit );
}

bool has_builder_bit( const Maze::Maze& maze, const Maze::Point& next )
{
  return maze[next.row][next.col] & Maze::builder_bit;
}

bool is_square_within_perimeter_walls( const Maze::Maze& maze, const Maze::Point& next )
{
  return next.row < maze.row_size() - 1 && next.row > 0 && next.col < maze.col_size() - 1 && next.col > 0;
}

void build_wall_line( Maze::Maze& maze, const Maze::Point& p )
{
  ...
  if ( p.row + 1 < maze.row_size() && !( maze[p.row + 1][p.col] & Maze::path_bit ) ) {
  ...
  if ( p.col + 1 < maze.col_size() && !( maze[p.row][p.col + 1] & Maze::path_bit ) ) {
    wall |= Maze::east_wall;
    maze[p.row][p.col + 1] |= Maze::west_wall;
  }
  maze[p.row][p.col] |= wall;
  maze[p.row][p.col] |= Maze::builder_bit;
  maze[p.row][p.col] &= static_cast<Maze::Square>( ~Maze::path_bit );
}

void build_wall_line_animated( Maze::Maze& maze, const Maze::Point& p, Speed::Speed_unit speed )
{
  Maze::Wall_line wall = 0b0;
  if ( p.row - 1 >= 0 && !( maze[p.row - 1][p.col] & Maze::path_bit ) ) {
    wall |= Maze::north_wall;
...
void build_path( Maze::Maze& maze, const Maze::Point& p )
{
  if ( p.row - 1 >= 0 ) {
    maze[p.row - 1][p.col] &= static_cast<Maze::Square>( ~Maze::south_wall );
  }
  if ( p.row + 1 < maze.row_size() ) {
    maze[p.row + 1][p.col] &= static_cast<Maze::Square>( ~Maze::north_wall );
  }
  if ( p.col - 1 >= 0 ) {
    maze[p.row][p.col - 1] &= static_cast<Maze::Square>( ~Maze::east_wall );
  }
  if ( p.col + 1 < maze.col_size() ) {
    maze[p.row][p.col + 1] &= static_cast<Maze::Square>( ~Maze::west_wall );
  }
  maze[p.row][p.col] |= Maze::path_bit;
}

void build_path_animated( Maze::Maze& maze, const Maze::Point& p, Speed::Speed_unit speed )
{
  maze[p.row][p.col] |= Maze::path_bit;
  flush_cursor_maze_coordinate( maze, p );
  std::this_thread::sleep_for( std::chrono::microseconds( speed ) );
...
void clear_for_wall_adders( Maze::Maze& maze )
{
  for ( int row = 0; row < maze.row_size(); row++ ) {
    for ( int col = 0; col < maze.col_size(); col++ ) {
...
void mark_origin( Maze::Maze& maze, const Maze::Point& walk, const Maze::Point& next )
{
...
void mark_origin_animated( Maze::Maze& maze,
                           const Maze::Point& walk,
...
void build_wall( Maze::Maze& maze, const Maze::Point& p )
{
  Maze::Wall_line wall = 0b0;
  if ( p.row - 1 >= 0 ) {
    wall |= Maze::north_wall;
...
void build_wall_carefully( Maze::Maze& maze, const Maze::Point& p )
{
  Maze::Wall_line wall = 0b0;
  if ( p.row - 1 >= 0 && !( maze[p.row - 1][p.col] & Maze::path_bit ) ) {
    wall |= Maze::north_wall;
...
void fill_maze_with_walls( Maze::Maze& maze )
{
  for ( int row = 0; row < maze.row_size(); row++ ) {
    for ( int col = 0; col < maze.col_size(); col++ ) {
...
void fill_maze_with_walls_animated( Maze::Maze& maze )
{
  Printer::clear_screen();
  for ( int row = 0; row < maze.row_size(); row++ ) {
    for ( int col = 0; col < maze.col_size(); col++ ) {
...
void carve_path_walls( Maze::Maze& maze, const Maze::Point& p )
{
  maze[p.row][p.col] |= Maze::path_bit;
  if ( p.row - 1 >= 0 ) {
    maze[p.row - 1][p.col] &= static_cast<Maze::Square>( ~Maze::south_wall );
...
void carve_path_walls_animated( Maze::Maze& maze, const Maze::Point& p, Speed::Speed_unit speed )
{
  maze[p.row][p.col] |= Maze::path_bit;
  flush_cursor_maze_coordinate( maze, p );
  std::this_thread::sleep_for( std::chrono::microseconds( speed ) );
...
void carve_path_markings( Maze::Maze& maze, const Maze::Point& cur, const Maze::Point& next )
{
  Maze::Point wall = cur;
  if ( next.row < cur.row ) {
    wall.row--;
...
void carve_path_markings_animated( Maze::Maze& maze,
...
void join_squares( Maze::Maze& maze, const Maze::Point& cur, const Maze::Point& next )
{
  Maze::Point wall = cur;
  build_path( maze, cur );
...
void join_squares_animated( Maze::Maze& maze,
                            const Maze::Point& cur,
                            const Maze::Point& next,
                            Speed::Speed_unit speed )
{
...
void add_positive_slope( Maze::Maze& maze, const Maze::Point& p )
{
...
void add_positive_slope_animated( Maze::Maze& maze, const Maze::Point& p, Speed::Speed_unit speed )
{
  const auto row_size = static_cast<float>( maze.row_size() ) - 2.0F;
  const auto col_size = static_cast<float>( maze.col_size() ) - 2.0F;
  const auto cur_row = static_cast<float>( p.row );
...
void add_negative_slope( Maze::Maze& maze, const Maze::Point& p )
{
  const auto row_size = static_cast<float>( maze.row_size() ) - 2.0F;
  const auto col_size = static_cast<float>( maze.col_size() ) - 2.0F;
  const auto cur_row = static_cast<float>( p.row );
...
void add_negative_slope_animated( Maze::Maze& maze, const Maze::Point& p, Speed::Speed_unit speed )
{
  const auto row_size = static_cast<float>( maze.row_size() ) - 2.0F;
  const auto col_size = static_cast<float>( maze.col_size() ) - 2.0F;
  const auto cur_row = static_cast<float>( p.row );
...
void build_wall_outline( Maze::Maze& maze )
{
  for ( int row = 0; row < maze.row_size(); row++ ) {
    for ( int col = 0; col < maze.col_size(); col++ ) {
      if ( col == 0 || col == maze.col_size() - 1 || row == 0 || row == maze.row_size() - 1 ) {
...
Maze::Point choose_arbitrary_point( const Maze::Maze& maze, Parity_point parity )
{
