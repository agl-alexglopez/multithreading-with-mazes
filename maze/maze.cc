...
///
/// The maze builder is responsible for zeroing out the direction bits as part
/// of the building process. When solving the maze we adjust how we use the
/// middle bits.
///
/// wall structure----------------------||||
/// ------------------------------------||||
...
/// that serves as the maze square. This is helpful for the multithreaded
/// solvers and allows for no synchronization primitives like mutexes for access
/// to the maze squares and the numerous bit operations that the solvers
/// perform. Now the finest grained locking possible is achieved with no need to
/// code any explicit critical sections of code as all use cases can be reduced
/// to loads and stores and compare and exchanges. The most permissive atomic
/// memory ordering is usually allowed as there is no purpose to the
/// multithreading other than more fun visuals.
...
class Square {
  public:
    ...
    ~Square() = default;
    constexpr Square(Square const &other);
    constexpr Square(Square &&other) noexcept;
    constexpr Square &operator=(Square const &other) noexcept;
    constexpr Square &operator=(Square &&other) noexcept;
    constexpr bool operator==(Square const &other) const noexcept;
    constexpr bool operator!=(Square const &other) const noexcept;
...
class Maze {
  ...
  public:
    explicit Maze(Maze_args const &args);
    std::span<Square> operator[](uint64_t index);
    std::span<Square const> operator[](uint64_t index) const;
    int row_size() const;
    int col_size() const;
    std::span<std::string_view const> wall_style() const;

  private:
    int maze_row_size_;
    int maze_col_size_;
    // Model a ROWxCOLUMN maze with a single flat array and manage indexing with
    // []operators.
...
constexpr Square::Square(Square const &other) : u16(other.u16.load()) {
}
...
constexpr bool
Square::operator==(Square const &other) const noexcept {
...
constexpr Square
Square::operator>>(uint16_t const n) const noexcept {
...
Maze::Maze(Maze_args const &args)
...
template <> struct hash<Maze::Point> {
    size_t
    operator()(Maze::Point const &p) const {
