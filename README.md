...
> **Note: This project is complete but also has a companion in Rust! See the new and improved Rust version ([multithreading-with-mazes-in-rust](https://github.com/agl-alexglopez/multithreading-with-mazes-in-rust))! Now, this project is primarily an experimental repository to try new ideas from recent C++ standard additions. For example, modules, `std::span`, `atomics`, and many other features make an appearance here. As I learn more C++, this repository will updated with improved techniques.**

In order to enjoy the mazes built and solved by this repository, you will need a terminal that supports ANSI escape sequences, 256 bit colors, and a Unicode compatible font installed for your terminal (see [Nerd Fonts](https://www.nerdfonts.com/)). Also, this project is intended to be built in a Unix-like environment, specifically I have tested it to work on any compiler which supports `ninja >= 1.11.1`, `clang++ >= 17.0.6`, and `cmake 3.28.1`. This limitation is due to my experimentation with C++ modules which are relatively new at the time of writing. Overall, I can't make promises for portability as this project is for my own personal enjoyment and is operating completely in terminal, without any 3rd party libraries. I will try to make the project more portable when time allows. Enjoy!
